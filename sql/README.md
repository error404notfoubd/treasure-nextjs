...
- **`users`**: marketing funnel signups (not `auth.users`) — encrypted phone/email, OTP state, survey fields, RLS. Includes `registration_step` enum. Lead flags **`contacted`**, **`has_replied`**, **`bonus_granted`** are coerced by trigger **`users_lead_flags_enforce_dependencies`**: if **`contacted`** is false then **`has_replied`** and **`bonus_granted`** are false; if **`has_replied`** is false then **`bonus_granted`** is false (same rules in **`PATCH /api/responses`**). Pool tables **`leads`** / **`customers`** follow **`has_replied`** only via **`sync_users_pool_membership`**.
- **`profiles.receive_verified_lead_notifications`**: dashboard opt-in for the **`notify-verified-lead`** edge function (verified survey leads). New signups default **false** (viewer); owners/admins/editors are set **true** when applying **`sql/migrations/20260422_profiles_receive_verified_lead_notifications.sql`**. Owners may toggle any profile in User Management; admins may toggle **editors** and **viewers** only (**`PATCH /api/users/notification-preference`**).
- **`app_settings`**: single-row (`id = 1`) tunables for the game economy, survey body size, OTP caps, QA control phone, dashboard auth rate limits, and **`facebook_page_url`** (official page for player emails; editable under Dashboard → System). Add the column on existing DBs with **`sql/migrations/20260423_app_settings_facebook_page_url.sql`**.
