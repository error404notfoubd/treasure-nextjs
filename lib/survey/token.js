...
/**
 * Create a signed token: "<surveyResponseId>.<timestamp>.<signature>"
 * The signature covers both the ID and the timestamp so neither can be altered.
 */
export function createVerificationToken(surveyResponseId) {
  const id = String(surveyResponseId);
...
/**
 * Verify and parse a token. Returns { valid, surveyResponseId } or { valid: false }.
...
export function verifyToken(token) {
  ...
  if (!/^\d+$/.test(id)) return { valid: false };
