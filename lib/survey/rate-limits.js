...
/** Per distinct phone (hashed), initial send + resends combined. */
export async function otpPerPhoneRateLimitOptions() {
  const s = await getAppSettings();
  return {
    max: Math.max(1, Number(s.otpSendsPerPhoneMax) || 3),
    windowMs: Math.max(60_000, Number(s.otpSendsPerPhoneWindowMs) || 3_600_000),
  };
}
