...
export default function SignupForm({ passwordMinLength, checkDebounceMs }) {
  ...
  const debouncedCheckName = (value) => {
    ...
    if (value.trim()) {
      nameTimer.current = setTimeout(() => checkAvailability("name", value), checkDebounceMs);
    }
  };
  ...
  const debouncedCheckEmail = (value) => {
    ...
    if (value.trim()) {
      emailTimer.current = setTimeout(() => checkAvailability("email", value), checkDebounceMs);
    }
  };
          ...
          <form onSubmit={handleSignup} aria-busy={loading}>
            <div className="mb-4">
              ...
              <input
                ...
                disabled={loading}
              />
              {checking.name && <p className="text-[11px] text-ink-4 mt-1">Checking…</p>}
            ...
            <div className="mb-4">
              ...
              <input
                ...
                disabled={loading}
              />
              {checking.email && <p className="text-[11px] text-ink-4 mt-1">Checking…</p>}
