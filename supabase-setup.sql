-- ═══════════════════════════════════════════════════════════════════════════
--  TREASURE HUNT — Single consolidated Supabase schema (idempotent)
--  Run the entire file in Supabase → SQL Editor.
--
--  Guarantees:
--    • Never drops public.profiles (or auth.users).
--    • Safe to run multiple times.
--    • RLS + grants on every application table in public schema below.
-- ═══════════════════════════════════════════════════════════════════════════


-- ─────────────────────────────────────────────
--  0. Schema baseline (least privilege; service_role + controlled client access)
-- ─────────────────────────────────────────────
...
-- ─────────────────────────────────────────────
--  1. PROFILES (never dropped — create-if-missing, then additive alters)
-- ─────────────────────────────────────────────
...
-- Re-apply check constraints (idempotent renames / fixes)
...
-- Signup: new users start as pending (adjust function only; trigger created below)
...
-- API-owned audit: no DB trigger on survey_responses
DROP TRIGGER IF EXISTS trg_survey_audit ON public.survey_responses;


-- ─────────────────────────────────────────────
--  3. VERIFICATION CODES (SMS OTP metadata; code stored as hash)
-- ─────────────────────────────────────────────
...
-- ─────────────────────────────────────────────
--  4. AUDIT LOG (append-only via rules; row_id text)
-- ─────────────────────────────────────────────
...
-- Optional trigger helper (not attached to survey_responses)
...
-- ─────────────────────────────────────────────
--  5. RATE LIMIT LOG (submission / IP tracking)
-- ─────────────────────────────────────────────
...
-- ─────────────────────────────────────────────
--  6. RATE LIMIT EVENTS (per-IP per-route; serverless-safe)
-- ─────────────────────────────────────────────
...
-- ─────────────────────────────────────────────
--  7. SECURE VIEW (invoker — inherits caller RLS; no direct client grants)
-- ─────────────────────────────────────────────
...
-- ─────────────────────────────────────────────
--  8. HELPER FUNCTIONS + EXECUTE GRANTS
-- ─────────────────────────────────────────────
...
-- Lock down function execution: service_role (+ authenticated only where needed)
